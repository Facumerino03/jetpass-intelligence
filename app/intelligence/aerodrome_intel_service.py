"""Aerodrome intelligence service — DB-first fetch with Document AI import fallback."""
...
async def get_aerodrome_intelligence(
    ...
    *,
    ...
    - Otherwise run import_aerodrome_from_aip (scrape → Document AI → persist)
    ...
    try:
        result = await import_aerodrome_from_aip(icao, output_dir=output_dir)
    except AipImportError as exc:
        alerts.append(Alert(level=AlertLevel.ERROR, code="IMPORT_FAILED", message=str(exc)))
        logger.error("[%s] Import failed: %s", icao, exc)
        return AerodromeIntelResult(
            icao=icao,
            data=None,
            source="fresh_import",
    ...
    logger.info("[%s] Import pipeline complete (airac=%s).", icao, result.current.meta.airac_cycle)

    return AerodromeIntelResult(
        icao=icao,
        data=result,
        source="fresh_import",
        airac_cycle=result.current.meta.airac_cycle,
        alerts=alerts,
        metadata={
            "sections_count": len(result.current.ad_sections),
        },
    )
